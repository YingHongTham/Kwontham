...
(recall that that means that some of the
interior dihedral angles $\theta^*$ are 0),
but we need the pyramids to be non-degenerate,
so we need to modify the graph on the torihedra
...
depending on whether the $\theta^*$ of that edges
...
and suppose it is left-augmented
(the other case is treated similarly).
Then for all left-augmented vertices $v_i$ of $\bar{f}$,
$f$ would meet the short,long edges $e_{i,0},e_{i,\pi}$,
while for all right-augmented vertices,
...
\begin{itemize}
\item \textbf{Case $k=n$:} (i.e. every vertex of $\bar{f}$ is left-augmented.)
...
\caption{TODO change this. Red edge indicates an added edge to $\Gamma_T(L)$;
the +/- labels indicate whether to
increase/decrease $\theta^*$ and $\vphi$.}
...
It is also easy to check that for each face of $\Gamma_T'$,
the $+/-$ labels cancel out,
so $\vphi'$ is an angle-splitting of it.
...
It is easy to see that, under the gluing of
$\sT_T(L)$ to $\sT_B(L)$,
...
It is also eashy to check that the $+/-$ labels get canceled out.
