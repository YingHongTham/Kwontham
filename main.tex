...
\begin{definition}
Let $D(K)$ be a twist reduced diagram of a link $K$ in $\torus \times I$, we define
\emph{augmenting} as a process in which an unknotted circle component,
called a \emph{crossing circle}, is added
to one or more twist regions of $D(K)$ (see Figure \ref{fig:Augmentations});
we call the resulting link $L$ an \emph{augmented link obtained from $K$}.
We say $L$ is \emph{fully} augmented
if $L$ is obtained by augmenting
...
between each crossing of our link
...
lying in the projection plane (except at crossings).
We now place a \emph{crossing edge} at each crossing of the original link $K$,
connecting the top and bottom strands at the crossing
(see Figure \ref{fig:crossingArc} left).
...
We view the link from the point at infinity from the top. We will push
...
We push down on the crossing circle and split the disk into
two identical disks.
...
crossing edges and edges from crossing circles.
Now, if there are half twists,
